...
public class ProfesionInputAdapterRest {
	...
	public ProfesionResponse actualizarProfesion(ProfesionRequest request) {
		try {
			setProfessionOutputPortInjection(request.getDatabase());
			Profession profession = professionInputPort.edit(Integer.parseInt(request.getId()), profesionMapperRest.fromAdapterToDomain(request));
			return profesionMapperRest.fromDomainToAdapterRestMaria(profession);
		} catch (InvalidOptionException | NoExistException e) {
			log.warn(e.getMessage());
		}
	...
	public Boolean eliminarProfesion(String database, String id) {
		try {
			...
			return professionInputPort.drop(Integer.parseInt(id));
		} catch (InvalidOptionException | NoExistException e) {
			log.warn(e.getMessage());
			return false;
		}
